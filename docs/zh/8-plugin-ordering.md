...
为了让用户和插件作者都能 **明确控制顺序**，从 v1.5.0（v1 LTS）/ v2.4.0（v2）开始引入 `priority` 字段。
...
| `BeforeSendPlugin`（v1.5.0+ / v2.4.0+） | `LATEST` (1000) | 全链路最终拦截，必须最后 |
...
从 v1.5.0（v1 LTS）/ v2.4.0（v2）开始，可按 name 获取已安装的插件实例：
...
如果只想临时调整一次、不愿继承，也可在已有实例上覆盖（注意 `priority` 是 `readonly`，需 cast）：
