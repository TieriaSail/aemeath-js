...
> 💡 **Privacy tip**: UploadPlugin has no built-in redaction. To filter / strip sensitive fields per entry, use the [`beforeSend` hook](./9-before-send.md) (v1.5.0+), which runs **before** UploadPlugin receives the entry.
