...
To let users and plugin authors **explicitly control order**, v2.4.0 introduced the `priority` field.
...
Available since v2.4.0. Look up an installed plugin by name:
...
For one-off / debugging use, you can also patch an existing instance (note `priority` is `readonly`, so a type cast is required):
