 ...
 *
 * 默认随 `initAemeath()` 安装。放在管道里所有采集类插件之后、`BeforeSendPlugin`
 * 之前，因此用户写 `beforeSend` 时面对的已经是"干净的骨架"，不必自己处理
 * Data URL / 二进制。
 *
 ...
 *
 * 详细文档参见 docs/{zh,en}/10-payload-sanitize.md
 */
...
export class PayloadSanitizePlugin implements AemeathPlugin {
  ...
  private noticeStrips(result: { strips: Array<{ path: string; kind: string; bytes: number }> }): void {
    ...
    console.info(
      `[Aemeath] Replaced an unloggable value at "${first.path}" (${first.kind}) with a short placeholder. `
        ...
        + 'Set `payloadSanitize: false` to disable (not recommended). (Shown once per session.)',
    );
  }
  ...
  private reportRejected(
    ...
    // 这是默认启用的插件里唯一会丢日志的路径，必须并入宿主已经在监听的那个出口。
