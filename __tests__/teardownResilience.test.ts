...
describe('拆卸链上任何一环抛出，都不能中断清理', () => {
  ...
  it('beforeunload 的反注册抛异常时，online 监听和 logger 引用仍要断干净', async () => {
    ...
    // 1.x 无 PlatformAdapter：直接挂 window.beforeunload；模拟 removeEventListener 翻脸
    ...
    expect((plugin as unknown as { boundHandleBeforeUnload: unknown }).boundHandleBeforeUnload).toBeNull();
  });
