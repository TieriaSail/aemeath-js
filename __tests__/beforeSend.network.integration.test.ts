...
/**
 * 顺序非常关键：
 *   1. 先把 window.fetch 替换成本次 mock
 *   2. **再** new NetworkPlugin / use（这样 monkey-patch 包装的是本次的 mock）
 *
 * 注意：main 分支的 NetworkPlugin 是自包含的（每个实例自己存 originalFetch、
 * 自己 patch / unpatch），所以测试结束必须 logger.destroy() 还原 fetch。
 */
