...
describe('UploadPlugin — 离线与恢复', () => {
  ...
  it('半开探测收到可恢复服务端失败并 parked 时也应退出半开', async () => {
    let reachable = false;
    const resumed = vi.fn();
    const plugin = new UploadPlugin({
      onUpload: async () => {
  ...
  it('暂停后的定时探测成功即自动恢复，无需 online 事件', async () => {
    ...
    const plugin = new UploadPlugin({
      onUpload: uploadFn,
      queue: { offlinePolicy: 'pause', deduplicationDelay: 10, suspectedOfflineThreshold: 2 },
      cache: { enabled: false },
      saveOnUnload: false,
    });
    logger.use(plugin);
  ...
  it('retryReason: network 不消耗重试预算', async () => {
    ...
    const plugin = new UploadPlugin({
      ...
      queue: { offlinePolicy: 'pause', maxRetries: 1, deduplicationDelay: 10, suspectedOfflineThreshold: 99 },
      cache: { enabled: false },
      saveOnUnload: false,
    });
    logger.use(plugin);
  ...
  it('队列溢出应触发 onDrop 而不是静默消失', async () => {
    ...
    const plugin = new UploadPlugin({
      onUpload: vi.fn().mockResolvedValue({ success: true } as UploadResult),
      queue: { offlinePolicy: 'pause', maxSize: 2, deduplicationDelay: 10000 },
      cache: { enabled: false },
      saveOnUnload: false,
      onDrop,
    });
