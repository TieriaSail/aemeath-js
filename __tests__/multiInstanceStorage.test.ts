...
describe('一个页面两个实例，默认配置', () => {
  ...
  it('让位是暂时的：撞车方走人后重装，缓存要能重新打开', async () => {
    ...
    const pluginA = new UploadPlugin({ onUpload: hang, queue: { deduplicationDelay: 0, uploadInterval: 100000 }, saveOnUnload: false });
    const pluginB = new UploadPlugin({ onUpload: hang, queue: { deduplicationDelay: 0, uploadInterval: 100000 }, saveOnUnload: false });
