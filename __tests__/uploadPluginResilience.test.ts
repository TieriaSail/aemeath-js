...
describe('UploadPlugin — 终止性保证', () => {
  ...
  it('抛出的 HTTP 错误（axios 风格）不算离线：耗预算丢弃，队列不暂停', async () => {
    ...
    const plugin = new UploadPlugin({
      onUpload: uploadFn,
      queue: { offlinePolicy: 'pause', deduplicationDelay: 10, maxRetries: 2, suspectedOfflineThreshold: 2 },
      cache: { enabled: false },
      saveOnUnload: false,
      onDrop: (_log, info) => dropped.push(info.reason),
    });
  ...
  it('回调自身的 bug（普通 Error）按服务端失败处理，不把链路挂起', async () => {
    ...
    const plugin = new UploadPlugin({
      onUpload: uploadFn,
      queue: { offlinePolicy: 'pause', deduplicationDelay: 10, maxRetries: 1, suspectedOfflineThreshold: 2 },
      cache: { enabled: false },
      saveOnUnload: false,
      onDrop: (_log, info) => dropped.push(info.reason),
    });
  ...
  it('恶劣的队列配置不会破坏终止性', async () => {
    ...
    for (const queue of hostile) {
      ...
      const plugin = new UploadPlugin({
        onUpload: uploadFn,
        queue: { offlinePolicy: 'pause', deduplicationDelay: 10, ...queue },
        cache: { enabled: false },
        saveOnUnload: false,
      });
      logger.use(plugin);
  ...
  it('缓存里缺少 retryCount 的历史数据不会变成无限重试', async () => {
    ...
    const plugin = new UploadPlugin({
      onUpload: uploadFn,
      queue: { offlinePolicy: 'pause', deduplicationDelay: 10, maxRetries: 2 },
...
describe('processQueue 崩溃不能变成自噬热循环', () => {
  ...
  it('一次 flush() 不会把所有日志的重试预算一次烧光', async () => {
    ...
    const plugin = new UploadPlugin({
      onUpload: uploadFn,
      queue: { offlinePolicy: 'pause', deduplicationDelay: 10, maxRetries: 3 },
      cache: { enabled: false },
      saveOnUnload: false,
