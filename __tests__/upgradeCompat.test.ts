 ...
 *
 * - **升级**：1.9 攒下的待发日志必须能被 1.10 恢复。1.9 没写 `cachedAt`，
 *   若 1.10 拿 `undefined` 去算年龄，所有老日志会被当成过期一把清掉 ——
 *   用户升级一次就丢一次待发队列。
 * - **回滚**：1.10 发现问题被回退到 1.9 时，1.9 必须能读懂 1.10 写的条目。
 *   多出来的字段可以忽略，但缺了 1.9 依赖的字段就会崩在恢复流程里。
 *
...
/** 一条完全按 1.9 格式写出来的缓存条目：没有 cachedAt，也没有 source */
...
describe('1.9 → 1.10 升级：老缓存必须能恢复', () => {
  ...
  it('1.9 写的缓存（没有 cachedAt）不能被 1.10 当成过期日志清空', async () => {
  ...
  it('1.9 缓存里真正过期的条目仍然按 TTL 丢弃，并且能报出来', async () => {
    localStorage.setItem(
      CACHE_KEY,
  ...
  it('1.9 缓存里 retryCount 已经用掉一部分时，预算要接着算而不是清零', async () => {
...
describe('1.9 → 1.10 升级：默认行为不能悄悄改变', () => {
  let logger: AemeathLogger;
  ...
  it('普通体量的日志经过默认清洗后必须一字不变', async () => {
    // PayloadSanitizePlugin 是 1.10 新增且**默认开启**的。对老用户来说这是
    // 一次无声的行为变更：如果它对正常日志也动手（截断、替换 Data URL、
    // 改 logId），后端拿到的数据就和 1.9 不一样了，而没人会预料到。
...
describe('1.10 → 1.9 回滚：新缓存不能让老版本崩', () => {
  let logger: AemeathLogger;
  ...
  it('1.10 写出的每一条都要满足 1.9 恢复流程依赖的字段', async () => {
    const plugin = new UploadPlugin({
      onUpload: async (): Promise<UploadResult> => ({
        success: false,
        shouldRetry: true,
    ...
    for (const item of parsed) {
      // 1.9 的 restoreFromCache 会读这四个字段，缺一个就会在恢复时出问题
      ...
      // 1.9 按 `now - timestamp < 1h` 过滤，timestamp 必须是入队时刻而不是别的
      expect(Date.now() - (item.timestamp as number)).toBeLessThan(60 * 60 * 1000);
    }
  });

  it('1.10 的拆分分片回滚到 1.9 后仍是合法条目（各自独立上报，不会崩）', async () => {
    const { PayloadSanitizePlugin } = await import('../src/plugins/PayloadSanitizePlugin');
    ...
    for (const item of parsed) {
      ...
      // logId 必须各不相同，否则 1.9 那边按 logId 去重会把分片吃掉
